...
function toggle_mustImplement()
{
	...
	if($('.mustImplement').parent().is(":visible")){
		$('#mnuRequired').text('Hide not required to implement');
	}
	else{
		$('#mnuRequired').text('Show not required to implement');
	}
...
function validateSubmit()
{
	...
	$fillins.each(function(){
		...
		if(answerid=='2454.100004300')
		{
			alert(xmlToString($(this).find('Response').get(0)));
		}
		if(!$answerElement.prop('checked') & $(this).find('Response').children(0).val()!='')
...
function SelectChoiceOnKeyPress(choiceID, element)
{
